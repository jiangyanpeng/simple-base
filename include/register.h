...
public:
    ...
    void Register(const std::string& key, Creator creator) {
        SIMPLE_LOG_DEBUG("will register %s creator", key.c_str());
        if (creator_map_.count(key)) {
            SIMPLE_LOG_WARN("type %s already registered", key.c_str());
            return;
        }
    ...
    std::shared_ptr<Base> Create(const std::string& key) {
        SIMPLE_LOG_DEBUG("will create %s creator", key.c_str());
        if (!creator_map_.count(key)) {
            SIMPLE_LOG_ERROR("type %s not register", key.c_str());
            return nullptr;
        }
